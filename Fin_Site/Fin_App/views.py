...
from .models import CustomUser, Space, SpaceLog, Shit, SpendingCategory, ReferalCode, Spending
from .serializers import UsersSerializer, SpaceSerializer, ConnectSerializer, CreatingSpaceSerializer, \
    SpendingCategorySerializer, ReferralCodeSerializer, ShitSerializer, SpendingSerializer, \
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def update_user_info(self, request):
        ...
        change_status = Shit.objects.get(user=change_user, space=space)

        if str(main_user.name) == str(change_status.user):
            return Response({'text': 'It is you'}, status.HTTP_200_OK)

        if change_status.user_perk == 'M':
            change_status.user_perk = 'C'
        else:
            change_status.user_perk = 'M'
        ...
        SpaceLog.objects.create(user=main_user, action=f'В SPACE "{space.title}" изменен статус пользователя '
                                                       f'{change_user.name} на {user_perk[change_status.user_perk]}')
        return Response(ShitSerializer(change_status).data, status.HTTP_200_OK)
    ...
    def ban_user(self, request):
        ...
        space = Space.objects.get(title=request.data['space_title'])
        ban_user_status = Shit.objects.get(user=ban_user, space=space, is_banned=request.data['is_banned'])
        ...
        return Response(ShitSerializer(ban_user_status).data, status.HTTP_200_OK)
...
class SpaceViewSet(viewsets.ModelViewSet):
    ...
    def space_create(self, request):
        ...
        Shit.objects.create(user=user, space=space, grade='A')
    ...
    def delete_user_from_space(self, request):
        ...
        del request.data['user_id']
        space = Space.objects.get(title=request.data['space'])
        delete_status = Shit.objects.get(user=user, space=space)
...
class ReferralCodeViewSet(viewsets.ModelViewSet):
    ...
    def generate_code(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        data = serializer.validated_data
        exist_code = ReferalCode.objects.filter(space=data['space'])

        if exist_code:
            ReferalCode.objects.get(space=data['space']).delete()

        code = ReferalCode.objects.create(user=data['user'], space=data['space'], code=request.data['code'])
    ...
    def join_space(self, request):
        ...
        try:
            code = ReferalCode.objects.get(code=request.data['code'])
        except Exception:
        ...
        if datetime.datetime.now() < code.expiration_time:
            ...
            try:
                Shit.objects.get(user=user, space=space)
            except Exception:
                join_space = Shit.objects.create(user=user, space=space, grade='C')
                SpaceLog.objects.create(user=user, action=f'"{user.name}" присоединился к "{space.title}"')
                return Response(ShitSerializer(join_space).data, status=status.HTTP_201_CREATED)
            else:
